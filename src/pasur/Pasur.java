...
public class Pasur extends Publisher
{
    ...
    public CompositeScoringStrategy compositeScoringStrategy = ScoringFactory.getInstance().getCompositeScoringStrategy();
            ...
            InstantiationException
    {
        ...
        this.addObserver(new PoolObserver());
    }
    ...
    public void play()
    {
        ...
        {
            roundOfGame++;
            publishEvent(this,"Round " + roundOfGame + " of the game starts...");
            ...
            while (!deckHand.isEmpty())
            {
                ...
                for(int i = 0; i < N_HAND_CARDS; i++)
                {
                    ...
                    for(int j = 0, k = currentStartingPlayerPos; j < nPlayers; j++)
                    {
                        ...
                        if(!cardsToPick.isEmpty())
                        {
                            ...
                            publishEvent(this,player.toString() + " picks " + toString(cardList));
                            ...
                            if(isAsur(playedCard, isLastRound))
                            {
                                ...
                                publishEvent(this,player.toString() + " scores a sur" );
                        ...
                        {
                            publishEvent(this,player.toString() + " picks " + toString(cardsToPick) );
                ...
                if(isLastRound)
                {
                    ...
                    if(!poolCards.isEmpty())
                        publishEvent(this,lastPlayerWhoPickedAcard + " picks " + toString(poolCards) + " at the end of this round of game");
            ...
            publishEvent(this,"Round " + roundOfGame + " of the game ends...");
            ...
            {
                Player player = players[i];
                if(player.getScore(compositeScoringStrategy) >= SCORE_TO_WIN)
                {
            ...
            }else
            {
                if(playersWithEnoughScore.size() > 1)
                {
                    ...
                    playersWithEnoughScore.sort((o1, o2) -> -Integer.compare(o1.getScore(compositeScoringStrategy), o2.getScore(compositeScoringStrategy)));
                    if(playersWithEnoughScore.get(0).getScore(compositeScoringStrategy) == playersWithEnoughScore.get(1).getScore(compositeScoringStrategy))
                    {
    ...
    private void updateScores()
    {
        ...
        for (int i = 0; i < nPlayers; i++)
        {
            ...
            Player player = players[i];
            scoreString += player.toString() + " = " + player.getScore(compositeScoringStrategy) + " (" + player.getSurs().getNumberOfCards() + " Surs)";
        }

        propertyChangePublisher.firePropertyChange(ON_UPDATE_SCORE, null, scoreString);
//        scoreLabel.setText(scoreString);
        publishEvent(this,"Total Running Scores: " + scoreString);
    }

    private void dealingOutToPlayers(int currentStartingPlayerPos)
    {

        publishEvent(this, "Dealing out to players...");
        ...
        for (int i = 0, k = currentStartingPlayerPos; i < nPlayers; i++)
        {
            ...
            if(k == nPlayers)
                k = 0;
            publishEvent(this,player.toString() + " hand: " + toString(player.getHand().getCardList()));
        }
    }

    private void dealingOutToPool()
    {
        publishEvent(this,"Dealing out to pool...");
        List<Card> cardList = new ArrayList<>(1);
        for (int i = 0; i < N_HAND_CARDS; i++)
        {
            if (paused) {
        ...
        publishEvent(this,"Pool: " + toString(poolHand.getCardList()));
    }
